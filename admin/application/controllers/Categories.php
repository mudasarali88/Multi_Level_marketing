...
class Categories extends CI_Controller {
        ...
        if ($this->session->userdata('loggedin') && $this->session->userdata('IsAdmin')) {
            ...
            if ($this->input->post() != NULL) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if ($insert) {
                        $this->session->set_flashdata('message', '<div class="alert alert-success">Plan created</div>');
                        redirect('categories');
                    }
    ...
    public function subCategories() {
         if ($this->session->userdata('loggedin')) {
             ...
             $data['subcategories'] = $this->Categories_model->getsubCategories();
             $data['parentSubcategories'] = $this->Categories_model->parentSubcategories();
            $data['categories'] = $this->Categories_model->getCategories();
            ...
            if ($this->input->post() != NULL) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    $this->load->view('adminpages/subcategories', $data);
                } else {
                    ...
                    $insert = $this->Categories_model->insertsubCategories($insertdata);
                    // var_dump($insertdata); die;
                    if ($insert) {
    ...
    public function EditsubCategories() {
         if ($this->session->userdata('loggedin')) {
            $data['categories'] = $this->Categories_model->getCategories();
            $data['subcategory'] = $this->Categories_model->subCategoriesByID($this->uri->segment(3));
            if ($data['subcategory']) {
            $data['subcategories'] = $this->Categories_model->getsubCategories();
            if ($this->input->post() != NULL) {
                ...
                $subcat_order=is_numeric ($this->input->post('subcat_order')) ? $this->input->post('subcat_order') : 0;
                    ...
                    $updatedata = array('subcat_title' => $this->input->post('subcat_title'),
                        ...
                        //var_dump($img); //die;
                    //var_dump($updatedata); die;
                    $updatedata = $this->Categories_model->updatesubCategories($updatedata,$this->uri->segment(3));
                    if ($updatedata) {
